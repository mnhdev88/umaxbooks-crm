/**
 * On-page SEO crawl — the checks behind /api/seo-audit.
 *
 * Lives in lib/ rather than the route so the SEO monitor cron and the report
 * builder can call it directly instead of HTTP-ing back into our own server.
 */
...
export async function auditUrl(rawUrl: string) {
  let url = rawUrl.trim()
...
/** Shape returned by auditUrl(); stored verbatim in seo_checks.onpage. */

export type AuditResult = Awaited<ReturnType<typeof auditUrl>>
