...
export function LeadCard({ lead, overlay, userRole, userId, agents = [], onReassign, callbackDate, emailSent, callLogged }: LeadCardProps) {
  ...
  // Show quick-email button only on New leads that have an email address
  const showEmailBtn = lead.status === 'New' && !!lead.email && !!userId
  ...
  return (
    <>
      ...
      >
        ...
        {/* Contact name + callback + GMB rating */}
      ...
      {/* Compose modal — rendered outside the card div so it's not clipped */}
      {composing && userId && (
        <ComposeModal
          leadId={lead.id}
