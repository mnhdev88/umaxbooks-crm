...
export async function POST(req: NextRequest) {
  const { leadId, demoUrl, companyName, developerName } = await req.json()
  if (!leadId || !demoUrl) {
    return NextResponse.json({ error: 'Missing required fields' }, { status: 400 })
  }

  // Automated email paused in Settings. The demo still lands in Approvals for admins.
...
<head>
  ...
  <style>
    ...
    .url-box { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 12px 16px; margin: 16px 0; word-break: break-all; font-size: 13px; color: #3b82f6; }
...
<body>
  <div class="wrapper">
    <div class="header">
      ...
      <div class="header-sub">CRM — Demo Approval Request</div>
    </div>
    <div class="body">
      <div class="greeting">Demo Ready for Review</div>
      <p class="text">
        <span class="biz">${developerName || 'A developer'}</span> has submitted a demo for
        <span class="biz">${companyName}</span> and it is awaiting your approval.
      </p>
      <p class="text">Demo URL:</p>
      <div class="url-box">${demoUrl}</div>
      <p class="text">Please review the demo and approve or decline it from the approvals page.</p>
      <a href="${approvalsUrl}" class="cta">Review Demo →</a>
      <p class="note">
  ...
  const results = await Promise.allSettled(
    admins.map(admin =>
      sendEmail({
        to: admin.email,
        subject: `Demo Approval Required — ${companyName}`,
        html,
      })
    )
  )
  ...
  return NextResponse.json({ success: true, anyFailed })
}
